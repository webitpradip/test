...
<?php $columnValue=isset($_GET['column_value'])?$_GET['column_value']:''; ?>
<form>
    Enter column value <input type="text" name="column_value" value="<?php echo $columnValue; ?>"/>
...
foreach($table2 as $table){
    ...
    foreach($columns as $column){
        ...
        $whereClause.=$column." like '%".$columnValue."%'";
    }
    $db1->query("select * from ".$table." where ".$whereClause);
    $rows=$db1->fetchAll();
    if(count($rows)>0){
