...
$sql = "select * from information_schema.processlist where DB='".$firstDbHost."'";
$sql.=" and info!=\"".$sql."\" and info is not null";
$db1->query($sql);
$rows = $db1->fetchAll();

if (count($rows) > 0) {
    $columns = array_keys($tables);
    createTable("Info Table", $columns, $rows);
}
