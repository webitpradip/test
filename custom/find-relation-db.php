...
$db4=new db($firstDbHost, $firstDbUser,  $firstDbPassword,  $firstDbName);
    ...
    foreach($tables as $table){
        ...
        $db3->query("SELECT  TABLE_SCHEMA,  TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME, REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME
        FROM
          INFORMATION_SCHEMA.KEY_COLUMN_USAGE
        WHERE REFERENCED_TABLE_SCHEMA = '".$firstDbName."' and TABLE_NAME = '".$table."'");
        $rows3 = $db3->fetchAll();

        if(count($rows3)>0){
            echo "<br/>Master Table / Foreign Key:<br/>";
            foreach($rows3 as $row3){
                echo "`".$row3['REFERENCED_TABLE_NAME']."`.`".$row3['REFERENCED_COLUMN_NAME']."`= `".$row3['TABLE_NAME']."`.`".$row3['COLUMN_NAME']."`<br/>";
            }
        }
        $db2->query("SELECT  TABLE_SCHEMA,  TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME, REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME
        FROM
          INFORMATION_SCHEMA.KEY_COLUMN_USAGE
        
        where REFERENCED_TABLE_SCHEMA = '".$firstDbName."' and REFERENCED_TABLE_NAME = '".$table."'");
          $rows2 = $db2->fetchAll();

        if(count($rows2)>0){
            echo "<br/>Transaction Tables:<br/>";
            foreach($rows2 as $row2){
            echo "`".$row2['TABLE_NAME']."`.`".$row2['COLUMN_NAME']."`=`".$row2['REFERENCED_TABLE_NAME']."`.`".$row2['REFERENCED_COLUMN_NAME']."`<br/>";
            }
        }
    }
