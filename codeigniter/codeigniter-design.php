...
if($_POST){
    if(!empty($_POST['wrapper'])){
        $wr2=$_POST['wrapper'];
        file_put_contents(realpath("./public/codeigniter/wrapper.txt"),$wr2);
    }
}
...
$checkbox = file_get_contents(realpath("./public/codeigniter/checkbox.txt"));
$date = file_get_contents(realpath("./public/codeigniter/date.txt"));
$datetime = file_get_contents(realpath("./public/codeigniter/datetime.txt"));
$radio = file_get_contents(realpath("./public/codeigniter/radio.txt"));
$select = file_get_contents(realpath("./public/codeigniter/select.txt"));
$text = file_get_contents(realpath("./public/codeigniter/text.txt"));
$textarea = file_get_contents(realpath("./public/codeigniter/textarea.txt"));
$label = file_get_contents(realpath("./public/codeigniter/label.txt"));
$file = file_get_contents(realpath("./public/codeigniter/file.txt"));
$multiselect = file_get_contents(realpath("./public/codeigniter/multiselect.txt"));
$password = file_get_contents(realpath("./public/codeigniter/password.txt"));
$wrapper = file_get_contents(realpath("./public/codeigniter/wrapper.txt"));
    ...
    foreach ($fieldName as $index => $processedField) {
        ...
        foreach ($wrappers as $wr) {
            ...
            if(strpos($wr,'errorMessage')){
                ...
                "<?"."php"." echo form_error('".$processedField."'); "."?>";;
            }
            $result .= $wr.PHP_EOL;
        }
    }
...
<h6>Codeigniter Design From Fields</h6>
    <form method="post">
        Fields::
        <br/>
